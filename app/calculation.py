...
def calculate_fi_metrics(
    ...
    real_monthly_return = (1 + monthly_investment_return) / (1 + monthly_inflation_rate) - 1
    real_monthly_return = real_monthly_return if real_monthly_return != 0 else 1e-10  # Prevent division by zero
    ...
    df["portfolio_nominal"] = np.array([
        ...
        monthly_savings * ((1 + monthly_investment_return) ** (i * 12) - 1) / monthly_investment_return
        for i in years
    ])

    df["portfolio_real"] = np.array([
        current_savings * (1 + real_monthly_return) ** (i * 12) +
        monthly_savings * ((1 + real_monthly_return) ** (i * 12) - 1) / real_monthly_return
        for i in years
    ])
    ...
    years_to_fi = df.loc[fi_achieved_mask, "years"].iloc[0] if fi_achieved_mask.any() else None

    # Step 10: Prepare the result metrics
    metrics = {
        "initial_fi_target": fi_target_initial,
        "years_to_fi": years_to_fi,
        "portfolio_at_fi": df.loc[fi_achieved_mask, "portfolio_real"].iloc[0] if years_to_fi else None,
        "monthly_savings_rate": (monthly_savings / (monthly_savings + monthly_expenses)) * 100,
        "fire_data": df,
    }
